...
async function migrate() {
  ...
  try {
    ...
    const applied = await client.query("SELECT 1 FROM lb_migrations WHERE version=$1", [version]);
    if (!applied.rowCount) {
      const sql = await readFile(path.join(process.cwd(), "db", "schema.sql"), "utf8");
      await client.query("BEGIN");
      await client.query(sql);
      await client.query("INSERT INTO lb_migrations(version) VALUES($1)", [version]);
